...
module.exports = {
  loaderController (modules) {
    const { root, rootConfig, configs, mode } = this.app
    ...
    for (let key in modules) {
      ...
      if (!singleMode && key === root) {
        continue
      }

      const moduleName = key
      const moduleRouteConfig = configs[key]
      ...
      for (let key in filesObj) {
        ...
        for (let key in moduleRoutes) {
          ...
          for (let pathKey in item) {
            ...
            // 设置根默认路由
            if (!singleMode && !setRootDefaultRoute && defaultRoute && defaultRoute.module === moduleName && defaultRoute.controller === controllerName && defaultRoute.action === key && actionMeta['params'].length === 0) {
              // 含params参数的默认路由取消设置
              if (actionMeta['params'].length > 0) {
                this.app.logger.app.warn('salak-core: The default route cannot contain the params.')
              } else {
